...
import image from "../Images/SVG_STAM.svg";
...
const StampPaper = () => {
  ...
  const fetchUserDetails = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8000/user/getuser_by_id/${id}`
      );
  ...
  //     const componentWidth = doc.internal.pageSize.getWidth() - 2 * marginLeft;
  //     const componentHeight = doc.internal.pageSize.getHeight() - 2 * marginTop;
    ...
    <>
      ...
      <HStack ml="6rem" mb="6rem"> 
        ...
        <Button
          ...
          _hover={{ background: "gray", color: "white" }}
        >
          {loader ? <span>Downloading</span> : <span>Downlode</span>}
        </Button>
      </HStack>
    </>
  );
